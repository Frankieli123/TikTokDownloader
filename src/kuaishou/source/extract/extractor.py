...
class HTMLExtractor:
    ...
    @staticmethod
    def _join_urls(value: object) -> str:
        if not value:
            return ""
    ...
    def _extract_audio_urls_app(self, data: Namespace) -> str:
        urls = data.safe_extract("music.audioUrls", [])
        if not urls:
            urls = data.safe_extract("soundTrack.audioUrls", [])
        return self._join_urls(urls)

    def _extract_audio_urls_web(self, state: SimpleNamespace, detail_key: str) -> str:
        for field in ("soundTrack", "music"):
            obj = Namespace.object_extract(state, f"{detail_key}.{field}", None)
            if not obj:
                continue
            ref = getattr(obj, "__ref", None)
            if ref:
                obj = Namespace.object_extract(state, ref, None)
            urls = getattr(obj, "audioUrls", None)
            if urls:
                joined = self._join_urls(urls)
                if joined:
                    return joined
            direct = getattr(obj, "url", None)
            if direct:
                return str(direct)
        return ""
    ...
    def __extract_detail_app(
        ...
        return {
            ...
            "audioUrls": self._extract_audio_urls_app(data),
        }
    ...
    def __extract_detail_web(self, data: Namespace, id_: str) -> dict:
        ...
        container["audioUrls"] = self._extract_audio_urls_web(data, detail)
        return container
...
class APIExtractor:
    ...
    def __extract_photo(self, item: dict, data: SimpleNamespace) -> None:
        ...
        item["userSex"] = self.USER_SEX.get(
            self.safe_extract(photo, "userSex"),
            "未知",
        )
        ...
        item["authorID"] = item["userEid"] or item["userId"]
        item["name"] = item["userName"]
        item["coverUrl"] = item.get("webpCoverUrls") or item.get("coverUrls") or ""
    ...
    def __extract_mp4(self, item: dict, data: SimpleNamespace) -> None:
        url = self.safe_extract(data, "mp4Url")
        item["download"] = [url] if url else []
    ...
    def __extract_atlas(self, item: dict, data: SimpleNamespace, index=0) -> None:
        ...
        item["download"] = [f"https://{cdn}{i}" for i in atlas]

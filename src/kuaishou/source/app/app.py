...
class KS:
    ...
    def __init__(
        ...
        *,
        ...
        config_data = self.config_obj.read()
        if cookie is not None:
            config_data = config_data | {"cookie": cookie}
        if proxy is not None:
            config_data = config_data | {"proxy": proxy}
    ...
    async def detail_one(
        ...
        if not data and user_id:
            data = await self.__handle_detail_api(user_id, detail_id)
        if not data:
            return _("获取作品数据失败")
    ...
    async def __handle_detail_api(
        ...
        data = self.extractor_api.run([raw] if raw else [])
        if not data:
            return None
        item = data[0]
        if not item.get("detailID") or item.get("detailID") == "Unknown":
    ...
    async def __save_data(
        ...
        async with recorder(self.manager, db_name=name, **params) as record:
            for i in data:
                download_value = i.get("download") or ""
                if isinstance(download_value, list):
                    download_value = " ".join(str(x) for x in download_value if x)
                record_data = i | {"download": download_value}
                await record.update(record_data)

...
class Downloader:
    ...
    async def __handle_music(
        ...
        if not self.music:
            return
        m = data.get("audioUrls")
        if isinstance(m, list):
            m = " ".join(str(i) for i in m if i)
        if not m:
            return
    ...
    async def __handle_video(
        ...
        if not self.__file_exists(file, "mp4"):
            if not data.get("download"):
                self.console.warning(_("《视频》{name} 下载链接为空").format(name=file.name))
                return
            tasks.append(
                self.__download_file(
                    data["download"][0],
    ...
    async def __handle_atlas(
        ...
        for index, url in enumerate(urls, start=1):
            ...
            if not (self.__file_exists(file, "webp") or self.__file_exists(file, "jpeg")):
                tasks.append(
                    self.__download_file(
                        url,
                        file,
        ...
        match self.cover:
            case "WEBP":
                if not self.__file_exists(path, "webp"):
                    tasks.append(
                        self.__download_file(
                            data.get("webpCoverUrls") or data.get("coverUrl") or data.get("coverUrls"),
                            path,
                            progress,
                            data["detailID"],
                            _("封面"),
            ...
            case "JPEG":
                if not self.__file_exists(path, "jpeg"):
                    tasks.append(
                        self.__download_file(
                            data.get("coverUrls") or data.get("coverUrl") or data.get("webpCoverUrls"),
                            path,
                            progress,
                            data["detailID"],
                            _("封面"),
